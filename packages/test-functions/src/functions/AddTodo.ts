import { registerFunction } from "@ulfschneider/azure-functions-openapi";
...
export async function AddToDo(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    return { status: 400, jsonBody: { code: 400, message: body.error.errors.map(err => `'${err.path}': ${err.message}`).join(', ') } as ErrorResponse };
};
