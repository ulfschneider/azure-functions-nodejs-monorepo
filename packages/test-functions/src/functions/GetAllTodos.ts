...
export async function GetAllTodos(
  ...
  const filterParams = FilterParamsSchema.safeParse(
    convertURLSearchParamsToObject(request.query)
  );
  ...
  if (filterParams.success) {
    ...
    const result = todos.slice(
      actualSkip,
      Math.min(todos.length, actualSkip + actualLimit)
    );
  ...
  return {
    ...
    jsonBody: {
      code: 400,
      message: filterParams.error.errors
...
registerFunction("GetAllTodos", "Get All Todos", {
  handler: GetAllTodos,
  methods: ["GET"],
  authLevel: "anonymous",
